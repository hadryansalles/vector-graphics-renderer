...
friend i_point_input_path<input_path_f_find_cubic_parameters<SINK>>;
    ...
    void do_cubic_segment(const R2 &p0, const R2 &p1, const R2 &p2,
        ...
        // ??D translate everyone so one of p0, p1, p2, p3 is zero?
		rvgf b3 = 3*det(p1, p2, p3);
		rvgf b2 = det(p0, p2, p3);
		rvgf b1 = det(p0, p1, p3);
		rvgf b0 = 3*det(p0, p1, p2);
        rvgf d1 = 2*det(b1,b0,b2,b1);
        rvgf d2 = det(b1,b0,b3,b2);
        rvgf d3 = 2*det(b2,b1,b3,b2);
        rvgf d = det(d1,d2,d2,d3); // Finally, the discriminant
        ...
        if (d > 0) {
            ...
            auto infls = std::make_tuple(b0, .25*(3*(b1+b2)-(b0+b3)), b3);
